...
server <- function(input, output, session) {
  ...
  boostDM_list = list(
    "colon" = fread(paste0("processed_data/boostdm/boostdm_genie_cosmic/colon_boostDM_cancer.txt.gz")),
    "lung" = fread(paste0("processed_data/boostdm/boostdm_genie_cosmic/lung_boostDM_cancer.txt.gz")),
    "blood" = fread(paste0("processed_data/boostdm/boostdm_genie_cosmic/CH_boostDM_cancer.txt.gz")))

  tissues = c("colon", "lung", "blood")

  # load metadata
  metadata_list  <- lapply(tissues, \(x)
                           fread(paste0("processed_data/", x, "/", x, "_metadata.tsv")) |>   distinct())
  expected_rates_list <- lapply(tissues, \(x)
                                fread(paste0("processed_data/", x, "/", x, "_expected_rates.tsv.gz")))
  ratios_list <-  lapply(tissues, \(x)
                         fread(paste0("processed_data/", x, "/", x, "_mut_ratios.tsv.gz")))
