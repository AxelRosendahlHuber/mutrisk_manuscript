...
Rcpp::sourceCpp("model_clones.cpp")
# read mutation rates:
mutrates = read.delim("~/Desktop/DNMT3A_HSC_rates.tsv")
B = 1e5
c = mutrates$slope[2]
start_rate = mutrates$intecept[2]
A = 0
s = 0.15
max_time = 70
...
model_multi_clones = function(nreps, B, s, max_time) {
  ...
  for (i in 1:nreps){
    list_trajectories[[paste0("sim_",i)]] = model_clones_rcpp(B = B, s = s, mrate = mrate, max_time = max_time, start_rate = start_rate)
  }
  data.frame(list_trajectories)
}

nsims = 100000
multi_clones = model_multi_clones(nreps = nsims, B = B, s = s, max_time = max_time) |>
